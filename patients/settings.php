    ...
    if (!$checkArchived->fetch()) {
        ...
        if (!empty($activityData) || !empty($visitationData)) {
            ...
            $insertDaily->execute([
             ...
             // Clear today's logs from main tables after archiving
...
<div class="h-[calc(100vh-5rem)] p-4 flex flex-col">
        ...
        <?php if ($currentSection === 'main_logs'): ?>
            <div class="bg-white rounded-2xl shadow-lg border border-clinic-tea/20 p-6 flex flex-col flex-1 min-h-0">
                <div class="mb-6">
                    ...
                    <p class="text-clinic-dark/60 mt-2">Consolidated view of all logs by date. Both activity and visitation logs are merged together and automatically archived daily, weekly, and monthly. Click "View Full" to see detailed logs for each type.</p>
                </div>
                ...
                <!-- Daily Logs Table -->
                <div id="dailyLogsTable" class="flex-1 flex flex-col min-h-0 border border-clinic-tea/20 rounded-lg overflow-hidden">
                ...
                <!-- Weekly Logs Table -->
                <div id="weeklyLogsTable" class="flex-1 flex flex-col min-h-0 border border-clinic-tea/20 rounded-lg overflow-hidden hidden">
                ...
                <!-- Monthly Logs Table -->
                <div id="monthlyLogsTable" class="flex-1 flex flex-col min-h-0 border border-clinic-tea/20 rounded-lg overflow-hidden hidden">
                    <table class="w-full text-sm min-w-[600px]">
                        <thead class="bg-clinic-ivory/50">
                            <tr>
...
<div id="logDetailsModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl max-w-6xl w-full max-h-[80vh] overflow-hidden">
...
function filterLogs() {
    ...
    // Filter all table bodies
    const tableBodies = ['dailyLogsTableBody', 'weeklyLogsTableBody', 'monthlyLogsTableBody'];
...
function showLogDetails(date, logType, logStatus = 'current') {
    ...
    fetch(`get_log_details.php?date=${encodeURIComponent(date)}&type=${encodeURIComponent(logType)}&log_type=${encodeURIComponent(logStatus)}`)
