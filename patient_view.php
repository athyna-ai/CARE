...
if (!$patient) {
    ...
    if (empty($patient['name'])) {
        ...
        header('Location: dashboard.php?error=incomplete_patient_data');
        exit;
    }
}
...
<div id="notificationContainer" class="fixed top-32 right-4 z-50 pointer-events-none"></div>
...
function showVisitationDetails(visitId) {
    ...
    iframe.src = `get_visitation_details.php?id=${visitId}`;
    iframe.onload = function() {
        try {
            ...
            fetch(`get_visitation_details.php?id=${visitId}`)
    ...
    if (confirm('Are you sure you want to archive this visitation record? It will be moved to the patient\'s archive.')) {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert('Error archiving visitation: ' + (data.message || 'Unknown error'));
                button.textContent = originalText;
                button.disabled = false;
            }
        })
        .catch(error => {
            alert('Error archiving visitation: ' + error.message);
            button.textContent = originalText;
            button.disabled = false;
        });
    }
