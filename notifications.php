...
<div class="min-h-screen bg-gradient-to-br from-clinic-ivory via-white to-clinic-vanilla p-4 md:p-6">
    <div class="w-full max-w-6xl mx-auto">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-clinic-dark">Notifications</h1>
            <p class="text-clinic-dark/70 mt-2">View and manage all system notifications</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white rounded-xl shadow-lg border border-clinic-tea/20 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-sm font-medium text-clinic-dark/60">Total</p>
            ...
            <div class="bg-white rounded-xl shadow-lg border border-clinic-tea/20 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-sm font-medium text-clinic-dark/60">Unread</p>
        ...
        <div class="flex flex-wrap gap-3 mb-6">
            <form method="POST" class="inline">
                ...
                <button type="submit" onclick="return confirm('Are you sure you want to mark all notifications as read?')" 
                        class="px-4 py-2 bg-clinic-blue hover:bg-clinic-blue/80 text-white rounded-lg font-medium transition-colors duration-200 flex items-center gap-2">
            ...
            <form method="POST" class="inline">
                ...
                <button type="submit" onclick="return confirm('Are you sure you want to clear all notifications? This action cannot be undone.')" 
                        class="px-4 py-2 bg-clinic-red hover:bg-clinic-red/80 text-white rounded-lg font-medium transition-colors duration-200 flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="logs/logs.php" class="px-4 py-2 bg-clinic-blue hover:bg-clinic-blue/80 text-white rounded-lg font-medium transition-colors duration-200 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button onclick="location.reload()" class="px-4 py-2 bg-clinic-tea hover:bg-clinic-tea/80 text-white rounded-lg font-medium transition-colors duration-200 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-clinic-tea/20">
            ...
            <div class="p-8 text-center">
                <svg class="w-16 h-16 mx-auto mb-4 text-clinic-tea/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                </svg>
                <h3 class="text-lg font-semibold text-clinic-dark mb-2">No Notifications</h3>
                <p class="text-clinic-dark/60">All caught up! No notifications to display.</p>
            </div>
            <?php else: ?>
            <div class="divide-y divide-clinic-tea/10">
                <?php foreach ($notifications as $notification): ?>
                <div class="notification-item p-4 hover:bg-clinic-ivory/30 transition-colors duration-200 <?= $notification['read'] ? 'opacity-75' : 'bg-clinic-ivory/20' ?>">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between mb-1">
                                <h4 class="text-sm font-semibold text-clinic-dark"><?= htmlspecialchars($notification['title']) ?></h4>
                                <div class="flex items-center gap-2">
                                    <span class="text-xs text-clinic-dark/60"><?= date('M j, Y g:i A', strtotime($notification['timestamp'])) ?></span>
                                    <?php if (!$notification['read']): ?>
                                    <div class="w-2 h-2 bg-clinic-red rounded-full"></div>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <p class="text-sm text-clinic-dark/80 mb-2"><?= htmlspecialchars($notification['message']) ?></p>
                            <?php if ($notification['action_url']): ?>
                            <a href="<?= htmlspecialchars($notification['action_url']) ?>" class="text-xs text-clinic-blue hover:text-clinic-blue/80 font-medium underline">
                                View Details →
                            </a>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="flex-shrink-0">
                            ...
                            <form method="POST" class="inline">
                                ...
                                <button type="submit" class="px-3 py-1 text-xs bg-clinic-blue hover:bg-clinic-blue/80 text-white rounded-lg transition-colors duration-200">
                                    Mark Read
                                </button>
                            </form>
                            <?php else: ?>
                            <span class="px-3 py-1 text-xs bg-gray-200 text-gray-600 rounded-lg">Read</span>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
