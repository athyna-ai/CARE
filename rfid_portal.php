...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($rfid === '') {
		...
		if ($student) {
			...
			log_activity($pdo, (int)$_SESSION['user']['id'], 'rfid_search', "Found student: {$student['name']} ({$student['info']})", 'rfid_portal');
			// Automatically redirect to patient view
			header("Location: patient_view.php?id={$student['id']}&type=student");
			exit;
		} elseif ($faculty) {
			// Log successful RFID search
			log_activity($pdo, (int)$_SESSION['user']['id'], 'rfid_search', "Found faculty: {$faculty['name']} ({$faculty['info']})", 'rfid_portal');
			// Automatically redirect to patient view
			header("Location: patient_view.php?id={$faculty['id']}&type=faculty");
			exit;
		} else {
			$result = ['exists' => false];
    ...
    <div id="roleModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-2xl mx-auto bg-white rounded-3xl shadow-2xl p-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <a id="roleStudent" class="group px-8 py-12 rounded-2xl bg-gradient-to-br from-clinic-blue to-clinic-tea text-white text-center hover:from-clinic-tea hover:to-clinic-blue transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl" href="#">
                    <div class="flex flex-col items-center gap-4">
                        <div class="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center group-hover:bg-white/30 transition-colors duration-300">
                        ...
                        <div>
                            <h3 class="text-2xl font-poppins font-bold">Student</h3>
                            <p class="text-sky-100 text-lg">Register a new student</p>
                        </div>
                    </div>
                </a>
                
                <a id="roleFaculty" class="group px-8 py-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-600 text-white text-center hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl" href="#">
                    <div class="flex flex-col items-center gap-4">
                        <div class="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center group-hover:bg-white/30 transition-colors duration-300">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            <h3 class="text-2xl font-poppins font-bold">Faculty</h3>
                            <p class="text-indigo-100 text-lg">Register a new faculty member</p>
                        </div>
                    </div>
                </a>
            </div>
            ...
            <div class="text-center">
                <button id="roleCancel" class="px-8 py-4 rounded-xl border-2 border-slate-300 text-slate-700 hover:border-slate-400 hover:bg-slate-50 transition-all duration-300 font-poppins font-medium text-lg">Cancel</button>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (roleStudent) {
        roleStudent.addEventListener('click', function(e) {
            e.preventDefault();
            const rfidValue = document.getElementById('rfidSearchInput').value;
            if (rfidValue) {
                window.location.href = `student_form.php?rfid=${encodeURIComponent(rfidValue)}`;
            } else {
    ...
    if (roleFaculty) {
        roleFaculty.addEventListener('click', function(e) {
            e.preventDefault();
            const rfidValue = document.getElementById('rfidSearchInput').value;
            if (rfidValue) {
                window.location.href = `faculty_form.php?rfid=${encodeURIComponent(rfidValue)}`;
            } else {
                window.location.href = 'faculty_form.php';
            }
        });
    }
